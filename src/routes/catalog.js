...
catalogRouter.post("/products", asyncRoute(async (req, res, next) => {
  ...
  const product = await withTransaction(async (client) => {
    ...
    if (req.actor.type === "supplier" && body.stockQty > 0) {
      await client.query(
...
catalogRouter.post("/products/:id/stock-movements", requireActorType("supplier"), asyncRoute(async (req, res) => {
  const body = z.object({
  ...
  const result = await withTransaction(async (client) => {
    ...
    if (product.supplier_id !== req.actor.id) throw new ApiError(403, "لا يمكنك تعديل صنف لا يخصك");
...
catalogRouter.get("/products/:id/stock-movements", requireActorType("supplier"), asyncRoute(async (req, res) => {
  const { rows: prodRows } = await query(`SELECT supplier_id FROM products WHERE id = $1`, [req.params.id]);
  if (!prodRows.length) throw new ApiError(404, "الصنف غير موجود");
  if (prodRows[0].supplier_id !== req.actor.id) throw new ApiError(403, "لا يمكنك الاطلاع على صنف لا يخصك");
  ...
  const result = await withTransaction(async (client) => {
    ...
    for (const row of inputRows) {
      ...
      if (existing.length) {
        ...
        await client.query(
          `INSERT INTO stock_movements (product_id, change_qty, reason, created_by)
           VALUES ($1,$2,'استيراد إكسل — تحديث مخزون',$3)`,
          [product.id, row.stockQty, req.actor.id]
        );
    ...
    if (updated.length) {
      await writeAudit(client, {
        ...
        action: "products.import_stock_updated", entityType: "product", entityId: req.actor.id,
        entityLabel: `تحديث مخزون ${updated.length} صنف عبر إكسل`, after: { count: updated.length }, ip: req.ip,
      });
    }

    return { updated, needsConfirmation, skipped };
  });
...
// تأكيد إضافة أصناف جديدة فعليًا (بعد ما راجعها المورد يدويًا من شاشة needsConfirmation)
catalogRouter.post("/products/import/confirm-new", requireActorType("supplier"), asyncRoute(async (req, res) => {
  const { rows: inputRows } = z.object({
  ...
  const result = await withTransaction(async (client) => {
    ...
    for (const row of inputRows) {
      const { rows } = await client.query(
        ...
        [row.sectionId, req.actor.id, row.name, row.unit, row.basePrice, row.stockQty, row.supplierSku]
      );
    ...
    await writeAudit(client, {
      actorType: "supplier", actorId: req.actor.id, actorName: req.actor.name,
      action: "products.imported_new", entityType: "product", entityId: req.actor.id,
  ...
  res.status(201).json({ createdCount: result.length, created: result });
}));
